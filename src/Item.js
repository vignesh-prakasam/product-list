            ...
            <div className={`sm:row-span-2 row-span-1 relative ${ props.addedToCart !== -1 ? ' border-2 border-red rounded-lg' : null }`}>
                ...
                <div className="flex flex-col justify-center items-center">
                {
                    ...
                    <span className="absolute transform -translate-x-1/5 -translate-y-1/5 h-10 w-2/3 bg-red border border-1 border-rose-400 rounded-3xl text-sm flex items-center justify-between"> 
                        <button className="rounded-full border border-white border-1 w-5 h-5 ml-5 hover:bg-white hover:border-red-500 group" onClick={() => props.reduceFromCart(props.item)}>
                            <svg xmlns="http://www.w3.org/2000/svg" className="p-1 w-4 h-4" width="10" height="2" fill="none" viewBox="0 0 10 2">
                            <path className="fill-white group-hover:fill-red-500" fill="#fff" d="M0 .375h10v1.25H0V.375Z"/>
                            </svg>
                        </button>
                        <span className="text-white text-sm font-semibold">{props.item.quantity}</span>
                        <button className="rounded-full border border-white w-5 h-5 mr-5 hover:bg-white hover:border-red-500 group" onClick={() => props.addToCart(props.item)}>
                            <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto w-3 h-3" fill="none" viewBox="0 0 10 10">
                                <path className="fill-white group-hover:fill-red-500" d="M10 4.375H5.625V0h-1.25v4.375H0v1.25h4.375V10h1.25V5.625H10v-1.25Z"/>
                            </svg>
                        </button>
                    ...
                    : 
                    <button className="absolute transform -translate-x-1/5 -translate-y-1/5 h-10 w-2/3 bg-white border border-1 border-rose-400 rounded-3xl text-sm flex items-center justify-center hover:border-red hover:text-red" onClick={() => props.addToCart(props.item)}> 
